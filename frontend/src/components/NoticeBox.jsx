...
function NoticeBox() {
  ...
  const fetchNotices = async () => {
    try {
      ...
      setError("There was an error fetching the notices. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
              ...
              noticesData.map((notice, index) => (
                <div key={index} className="py-2 h-fit">
                  <Link
                    ...
                    to={currentUser?.role === 'student' ? `/student/notice/${notice?._id}` : currentUser?.role === 'tpo_admin' ? `/tpo/notice/${notice?._id}` : currentUser?.role === 'management_admin' ? `/management/notice/${notice?._id}` : ''}
                  >
                    {notice?.title}
